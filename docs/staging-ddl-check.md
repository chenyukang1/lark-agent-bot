...
`build_number` 必须为正整数。phase 为可选字段，目前不用于过滤回调。
...
- 使用现有 DASHSCOPE_API_KEY / DASHSCOPE_API_HOST 和 qwen-max 直接生成 Markdown 同步提醒。
...
- 每项 DDL 变动生成一份 Markdown 报告，通过 SQL 通知卡片逐条私聊；同一 open_id 可收到多条提醒。
  报告中的 commit 和文件路径必须匹配本次构建，收件人始终取自实际 Git 作者。
  需配置 SQL_NOTICE_CARD_ID，模板使用 report_content 和 alarm_time 变量。
  不向 NOTIFY_CHAT_ID 发群通知。
...
去重保留当前单进程内最近 1,000 个构建，按每条提醒记录发送状态，并缓存分析结果；同一构建重试时只发送未成功的提醒。
